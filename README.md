...
4. **vman_dq data-quality filter** (new) — calls [`vman_dq.compute_ici()`](https://github.com/vman-tool/vman3_dq) directly (not reimplemented) to drop records where the raw interview data itself is contradictory on a pregnancy/maternal-vs-sex question (rules C1/C8), plus any record in vman_dq's own "Critical" ICI tier (< 70%) → **1** dropped.
   - Note: vman_dq's **C7 rule is currently mislabeled** — it treats `id10109`/`id10110` as "pregnancy-related symptoms reported for a male," but those fields actually ask whether a delivered baby moved/breathed after birth (stillbirth/live-birth questions), not a pregnancy symptom. Using it as an exclusion criterion would incorrectly drop genuine male neonatal deaths (~400 of ~12k rows here). `vman_ml` deliberately excludes C7 from this filter until it's fixed upstream in `vman_dq/dqa.py`.
